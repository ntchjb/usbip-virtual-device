...
func TestStandardDeviceDescriptor(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			obj: &protocol.StandardDeviceDescriptor{
				BLength:            protocol.STANDARD_DEVICE_DESCRIPTOR_LENGTH,
				BDescriptorType:    protocol.DESCRIPTOR_TYPE_DEVICE,
				BCDUSB:             0x1000,
			...
			newObj: func() usbipprot.Serializer {
				return &protocol.StandardDeviceDescriptor{}
			},
			encErr: nil,
			decErr: nil,
		},
